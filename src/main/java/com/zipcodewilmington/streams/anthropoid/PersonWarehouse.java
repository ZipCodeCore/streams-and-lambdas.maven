 ...
 */
    ...
    public static Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        return getUniquelyNamedPeople().filter(person -> person.getName().substring(0).equals(character));
    ...
    public static Map<Long, String> getIdToNameMap() {
        Map<Long, String> idToNameMap = people.stream().collect(Collectors.toMap(p-> p.getPersonalId(), p -> p.getName() ));
        return idToNameMap;
